...
from statisticscore.models import Session, Committee, SubTopic, ActiveDebate, \
                              ActiveRound
from statisticscore.forms.running_order import PredictForm, PredictEditForm
...
def predict(request, session_id, committee_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(reverse('statisticscore:predict', args=[session_id, committee_id]))
    ...
    return check_authorization_and_render(request, 'statisticscore/predict_form.html', context, session, False)
