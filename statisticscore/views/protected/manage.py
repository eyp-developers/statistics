...
from statisticscore.models import Session, Committee, SubTopic, ActiveDebate, \
                              ActiveRound
from statisticscore.forms.active import ActiveDebateForm, ActiveRoundForm
...
def manage(request, session_id):
    ...
    if request.method == 'POST':
        ...
        if 'active_debate' in request.POST:
            ...
            if debate_form.is_valid():
                ...
                return HttpResponseRedirect(reverse('statisticscore:manage', args=[session_id]))
    ...
    # Otherwise, give the User some nice new forms.
        ...
        debate_form = ActiveDebateForm(committees_array, {'session': session.name})
        round_form = ActiveRoundForm(max_rounds_array, {'session': session.name})
        point_form = PointEditForm([], {'session': session.name})
        content_form = ContentEditForm({'session': session.name})
        vote_form = VoteEditForm({'session': session.name})
    ...
    return check_authorization_and_render(request, 'statisticscore/manage.html', context, session)
