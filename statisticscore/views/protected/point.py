...
from statisticscore.models import Session, Committee, SubTopic, ActiveDebate, \
                                ActiveRound, Point
from statisticscore.forms.point import PointForm
...
def point(request, session_id, committee_id=None):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if all_form:
                return HttpResponseRedirect(reverse('statisticscore:point_all', args=[session_id]))
            else:
                return HttpResponseRedirect(reverse('statisticscore:point', args=[session_id, committee_id]))
    ...
    return check_authorization_and_render(request, 'statisticscore/point_form.html', context, session, False)
