...
# This view is shown, when a user tries to view any protected page, but isn't logged in. After they log in, they'll be taken to the appropriate place.
        ...
        if user is not None:
            if user.is_active:
                ...
                if is_safe_url(next, settings.ALLOWED_HOSTS):
                    ...
                    return HttpResponseRedirect(reverse('statisticscore:session', args = [session.pk]))
                    ...
                    return HttpResponseRedirect(reverse('statisticscore:home'))
                ...
                return HttpResponseRedirect(reverse('statisticscore:login'))
    ...
    template = 'statisticscore/login.html'
    context = {'form': form}
